export default function Body() {
  return (
    <>
      <p>
        Le tatouage est le métier où un pourboire dématérialisé rapporte le plus par
        client, et de loin. Ticket élevé, prestation longue, relation personnelle : quand
        le pourboire tombe, il ne fait pas 2 € mais 20 à 40 €.
      </p>

      <h2>Pourquoi le tatouage se prête si bien au pourboire</h2>
      <p>
        Trois choses se combinent, et on les retrouve rarement ensemble.
      </p>
      <p>
        <strong>Le ticket est élevé.</strong> Sur une séance à 300 €, laisser 20 € ne se
        ressent pas de la même façon que 20 € sur un café. Le pourboire suit le montant de
        la prestation, pas une habitude fixe.
      </p>
      <p>
        <strong>La relation est longue et personnelle.</strong> Deux à six heures en
        tête-à-tête, sur un projet que le client a préparé pendant des semaines. Ce n&apos;est
        pas une transaction, c&apos;est une collaboration — et ça se voit au moment de payer.
      </p>
      <p>
        <strong>La culture existe déjà,</strong> importée des studios américains où le
        pourboire au tatoueur est une norme. Elle est moins systématique en France, mais
        elle n&apos;est pas à créer.
      </p>

      <h2>Ce que ça représente concrètement</h2>
      <p>
        Deux à trois clients par jour, un pourboire moyen autour de 20 € quand il est
        laissé : on parle de plusieurs centaines d&apos;euros par mois. Sur un métier où le
        matériel, l&apos;encre et la location de poste pèsent lourd, ce n&apos;est pas un
        détail.
      </p>
      <p>
        Contrairement à un serveur, vous êtes le plus souvent votre propre patron. La somme
        ne transite par personne : elle arrive sur votre compte, pour votre travail.
      </p>

      <h2>Le vrai problème : le cash a disparu de votre clientèle</h2>
      <p>
        Votre clientèle est jeune, urbaine, et paie par carte ou par téléphone. Le geste du
        pourboire n&apos;a pas disparu parce que les gens sont devenus moins reconnaissants
        — il a disparu parce que sortir un billet suppose d&apos;en avoir un.
      </p>
      <p>
        Le résultat est le même dans tous les studios : le client dit « c&apos;est génial,
        merci beaucoup », paie sa séance par carte, et repart. L&apos;envie était là, le
        moyen n&apos;y était pas.
      </p>

      <h2>Où poser la plaque</h2>
      <p>
        Sur le comptoir d&apos;accueil, à côté du terminal de paiement. C&apos;est là que
        se fait le règlement de fin de séance, et c&apos;est le seul moment où le client a
        son téléphone en main.
      </p>
      <p>
        Deuxième emplacement qui marche bien : à côté du miroir, là où le client regarde
        le résultat avant de partir. C&apos;est le moment d&apos;émotion maximale de toute
        la séance — bien plus fort que le moment du paiement.
      </p>
      <p>
        Si le studio compte plusieurs artistes, chacun peut avoir son propre profil : le
        client choisit qui il récompense, et la somme part sur le compte de cette personne
        sans passer par le studio.
      </p>

      <h2>Pourquoi une plaque plutôt que le terminal</h2>
      <p>
        Un terminal qui affiche « souhaitez-vous ajouter un pourboire ? » juste après une
        séance à 300 €, sous le regard de l&apos;artiste, met tout le monde mal à l&apos;aise.
        Le client vient de dépenser une somme importante ; lui demander frontalement plus
        d&apos;argent casse le moment.
      </p>
      <p>
        Une plaque posée ne demande rien. Le client la voit, décide seul, et son refus ne
        s&apos;affiche sur aucun écran. C&apos;est toute la différence entre proposer et
        solliciter — et dans un métier qui repose sur la relation, elle compte.
      </p>

      <h2>Le statut : à vérifier avec votre comptable</h2>
      <p>
        L&apos;exonération d&apos;impôt et de cotisations sur les pourboires, prolongée
        jusqu&apos;au 31 décembre 2028, vise les <em>salariés</em> en contact avec la
        clientèle rémunérés sous 1,6 SMIC.
      </p>
      <p>
        La plupart des tatoueurs exercent en indépendant, souvent en location de poste :
        vous n&apos;êtes alors pas salarié du studio, et les sommes que vous percevez
        relèvent de votre propre régime. Ne vous fiez pas à cette page pour votre situation
        — faites-la vérifier. Le dispositif n&apos;est pas ce qui rend le pourboire
        intéressant chez vous ; ce sont les montants.
      </p>

      <h2>Ce que ça coûte</h2>
      <p>
        La plaque est un achat unique, sans abonnement. Aucun prélèvement sur les pourboires : le client ajoute des frais de service (0,25 € + 5 % du pourboire) et vous recevez 100 % du pourboire. Sans
        pourboire, vous ne payez rien.
      </p>
      <p>
        À l&apos;échelle des montants du tatouage, l&apos;arbitrage est simple : un seul
        pourboire de 20 € rembourse une bonne partie de la plaque. Un abonnement mensuel
        fixe, lui, tournerait aussi pendant vos semaines creuses ou vos conventions.
      </p>
    </>
  );
}
