  ...
  switch (event.type) {
    ...
    case 'checkout.session.completed': {
      ...
      if (!groupId && session.metadata?.source === 'express' && pack) {
        ...
        if (expressPromoCode && newOrder?.id) {
          await attributeAmbassadorSale(
            supabase, { promoCodeStr: expressPromoCode },
      ...
      if (promoCodeId && upsertedOrder?.id) {
        await attributeAmbassadorSale(
          supabase, { promoCodeId },
...
async function attributeAmbassadorSale(
  supabase: ReturnType<typeof createServiceClient>,
  lookup: { promoCodeId: string } | { promoCodeStr: string },
  orderId: string,
  pack: 'solo' | 'duo',
  rawSalonName: string
): Promise<void> {
  try {
    ...
    if ('promoCodeId' in lookup) {
      promoCodeId = lookup.promoCodeId;
    } else {
      const { data: pc } = await supabase
        .from('promo_codes')
        .select('id')
        .eq('code', lookup.promoCodeStr.toUpperCase())
        .maybeSingle();
      promoCodeId = pc?.id ?? null;
    }
    ...
    // Never break the webhook — attribution is best-effort
  }
}
