...
export async function GET(
  ...
  const { data: contract } = await service
    ...
    .select('id, commercial_id, title, content_snapshot, content_hash, status, signed_at, signature_image_path, signer_ip_hash')
    .eq('id', id)
    .maybeSingle();
  ...
  const html = buildSignedContractPage({
    ...
    signatureDataUrl,
    ambassadorName: session.name, // helper key is generic — used as "signer name"
