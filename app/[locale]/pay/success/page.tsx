...
export default async function PaySuccessPage({ params, searchParams }: Props) {
  ...
  const statusBadge =
    status === 'succeeded'
      ...
      : status === 'processing'
        ...
        : { label: t('failed'), fg: 'var(--error)', bg: 'var(--surface-2)' };
  ...
  return (
    <main style={{
      ...
      <div style={{ position: 'fixed', top: '20%', left: '50%', transform: 'translateX(-50%)', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(34,197,94,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
