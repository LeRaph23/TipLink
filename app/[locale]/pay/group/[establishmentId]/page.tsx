      ...
      <div style={{
        ...
        {salonName === null ? (
          <div
            className="shimmer"
            aria-hidden
            style={{ height: 15, width: 140, margin: '8px auto 2px', borderRadius: 7, background: 'rgba(255,255,255,0.35)' }}
          />
        ) : (
...
/* Instant shell: branded hero (real headline) + a few shimmer rows matching the
   real staff cards exactly, so there is no layout shift when data streams in. */
function GroupTipSkeleton({ t }: { t: T }) {
  return (
    <>
      <HeroCard t={t} salonName={null} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }} aria-hidden>
        {[0, 1, 2].map((i) => (
          ...
          >
            ...
            <div className="shimmer" style={{ height: 18, width: `${60 - i * 8}%`, borderRadius: 7 }} />
          </div>
        ))}
      </div>
      <PoweredBy t={t} />
