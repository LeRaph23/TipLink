  ...
  return (
    <div style={{ width: size, height: size, borderRadius: 24, background: '#fff', border: '1.5px solid #e4e4ec', boxShadow: '0 20px 60px rgba(0,0,0,0.10), 0 4px 16px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, flexShrink: 0 }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 11, fontWeight: 800, color: '#111118', letterSpacing: '0.06em' }}>TIPLINK</div>
        <div style={{ fontSize: 9, color: '#74748a', letterSpacing: '0.04em' }}>NFC SmartTag</div>
      </div>
    </div>
  );
}
  ...
  return (
    <header style={{ position: 'sticky', top: 38, zIndex: 200, height: 62, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 clamp(16px, 4vw, 48px)', background: scrolled ? 'rgba(255,255,255,0.96)' : '#fff', backdropFilter: scrolled ? 'blur(12px)' : 'none', borderBottom: '1px solid #e4e4ec', transition: 'background 300ms' }}>
      <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
        <LogoMark size={26} />
        <span style={{ fontWeight: 900, fontSize: 16, letterSpacing: '-0.03em', color: '#111118' }}>TipLink</span>
      </Link>
...
function ProductSection({ onOrderClick }: { onOrderClick: (p: 's' | 'm' | 'l') => void }) {
  const t = useTranslations('landing');
  return (
...
// ─── Product grid ─────────────────────────────────────────────────────────────
function ProductGridSection({ onOrderClick }: { onOrderClick: (p: 's' | 'm' | 'l') => void }) {
  const t = useTranslations('landing');
  const packs = [
    { key: 's' as const, name: t('grid.packS'), tags: t('grid.packSTag'), price: t('grid.packSPrice'), full: t('grid.packSFull'), save: '40%' },
    { key: 'm' as const, name: t('grid.packM'), tags: t('grid.packMTag'), price: t('grid.packMPrice'), full: t('grid.packMFull'), save: '42%', popular: true },
    { key: 'l' as const, name: t('grid.packL'), tags: t('grid.packLTag'), price: t('grid.packLPrice'), full: t('grid.packLFull'), save: '38%' },
  ];
  return (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 32, marginBottom: 40 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
              <LogoMark size={24} />
              <span style={{ fontWeight: 900, fontSize: 15, color: '#fff', letterSpacing: '-0.02em' }}>TipLink</span>
            </div>
...
export default function LandingPage() {
  ...
  const [cartPack, setCartPack] = useState<'s' | 'm' | 'l' | null>(null);
  const openCart = (pack: 's' | 'm' | 'l' = 'm') => setCartPack(pack);
