...
export function GroupAmountSelector({ establishmentId, currency, thresholds, staffCount }: Props) {
  ...
  const fmt = new Intl.NumberFormat(undefined, { style: 'currency', currency, minimumFractionDigits: 0 });
  const fmtCents = new Intl.NumberFormat(undefined, { style: 'currency', currency, minimumFractionDigits: 2 });
  ...
  return (
    <>
      ...
      {hasAmount && tipAmount && (
        <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-3)', margin: '-4px 0 0', lineHeight: 1.6 }}>
          {fmt.format(tipAmount / 100)} pourboire{perPerson ? ` (${perPerson} / pers.)` : ''}&nbsp;+&nbsp;{fmtCents.format(SERVICE_FEE_CENTS / 100)} frais
          {' = '}
