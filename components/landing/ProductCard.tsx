...
function PlaqueSVG({ bg = '#f5f3ff', accent = '#7c3aed', size = 200 }: { bg?: string; accent?: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" fill="none">
      <rect width="200" height="200" rx="24" fill={bg} />
      <rect x="50" y="50" width="100" height="100" rx="18" fill={accent} fillOpacity="0.1" stroke={accent} strokeOpacity="0.25" strokeWidth="1.5" />
      ...
      <path d="M71 100c0-16 13-29 29-29" stroke={accent} strokeWidth="2" strokeLinecap="round" strokeOpacity="0.35" />
      <path d="M129 100c0 16-13 29-29 29" stroke={accent} strokeWidth="2" strokeLinecap="round" strokeOpacity="0.35" />
      {/* Epoxy texture hint */}
      <rect x="50" y="162" width="100" height="8" rx="4" fill={accent} fillOpacity="0.08" />
      <text x="100" y="185" textAnchor="middle" fontSize="9" fontWeight="700" fill={accent} fillOpacity="0.5" fontFamily="sans-serif" letterSpacing="2">DIGITIP</text>
    </svg>
  );
}
    ...
    <div style={{
      ...
      <div>
        <div style={{
          ...
          <PlaqueSVG bg={GALLERY_CONFIGS[activeImg].bg} accent={GALLERY_CONFIGS[activeImg].accent} size={240} />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {GALLERY_CONFIGS.map((cfg, i) => (
            <button key={i} onClick={() => setActiveImg(i)} style={{
              width: 56, height: 56, borderRadius: 10, border: activeImg === i ? '2px solid #7c3aed' : '1px solid #e6e6f0',
              background: cfg.bg, cursor: 'pointer', padding: 4, flexShrink: 0,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <PlaqueSVG bg={cfg.bg} accent={cfg.accent} size={40} />
            </button>
          ))}
        </div>
      </div>
